...
brew install git golang node kubernetes-cli mysql-client git-town \
  skaffold helm circleci cockroach composer gettext terraform \
  sonatype-nexus-community/homebrew-nancy-tap/nancy cockroachdb/tap/cockroach \
  gh gnupg gnutls wget sqlite postgresql golangci-lint goreleaser hashcat heroku/brew/heroku imagemagick minikube
